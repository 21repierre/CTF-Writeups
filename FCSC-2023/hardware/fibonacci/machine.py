...
class Machine:
    ...
    def fetchInstruction(self):
        ...
        if not self.instruction:
            print(f"Wrong instruction {opcode}")
            self.error = True
    ...
    def runCode(self):
        while True:
            #print(f"R0={self.R0} ; R1={self.R1} ; R2={self.R2} ; R5={self.R5}")
            t = self.pc
            self.fetchInstruction()
            #print(f"{self.nbInstruction} - {self.instruction}")
            ...
            if self.pc >= self.code_size or self.nbInstruction > (1 << 16):
                self.end = True
                self.error = True
                print("jsp error", self.pc >= self.code_size, self.nbInstruction > (1 << 16))
                return self
    ...
    def div(self):
        if self.b != 0:
            ...
            print(f"DIV ERROR a={self.a} ; b={self.b}")
            self.error = True
